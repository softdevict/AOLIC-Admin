...
import { delete_action, delete_footer_contact_us, update_action, update_card, update_footer_contact_us } from '../../api/config';
...
const ContactUsEditForm = () => {
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            formData.append('contactName', data.action);
            formData.append('contactLink', data.link);

            // Ensure img exists before appending
            if (data.img && data.img.length > 0) {
                formData.append('contactImage', data.img[0]); // Append the first file
            }
            ...
            const response = await axios.patch(`${update_footer_contact_us}/${id}`, formData, {
                headers: {
    ...
    const deleteAction = async () => {
        try {
            const response = await axios.delete(`${delete_footer_contact_us}/${id}`);
        ...
        <div>
                      <ol className="flex text-gray-500 font-semibold dark:text-white-dark space-x-2">
                          ...
                          <Link to="/footer">
                            ...
                            <li>/</li>
                            <li>
                                <button className="text-black dark:text-white-light hover:text-black/70 dark:hover:text-white-light/70">Contact Us</button>
                            </li>
