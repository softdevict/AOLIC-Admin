...
import { my_dashboard_1, my_dashboard_all_users } from "../../../api/config";
...
const First_My_Dashboard_Add: React.FC = () => {
    const { root_Card_1 } = useParams<{ root_Card_1: string }>();
    const dashboardId = root_Card_1 || "default-dashboard-id";
    ...
    useEffect(() => {
        const fetchAllUsers = async () => {
            try {
                const response = await axios.get(`${my_dashboard_all_users}/${dashboardId}`);
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            toast.success("Dashboard card added successfully!", {
                position: "top-right",
                autoClose: 3000,
                onClose: () => navigate(`/my_dashboard_1/${dashboardId}`),
            });
        } catch (error) {
    ...
    return (
        ...
        <>
            <ol className="flex text-gray-500 font-semibold dark:text-white-dark space-x-2">
                ...
                <li>/</li>
                <Link to={`/my_dashboard_1/${dashboardId}`}>
            ...
            <div className="p-6 max-w-xl mx-auto bg-white rounded-2xl shadow">
                <div className="flex px-2 justify-between">
                    ...
                    <ClearIcon
                        onClick={() => navigate(`/my_dashboard_1/${dashboardId}`)}
                ...
                {cards.length > 0 && (
                    <div className="mb-6">
                        <Typography variant="h6" className="mb-2 font-semibold">Added Dashboard Cards</Typography>
                        <div className="space-y-2">
                            {cards.map((card) => (
                                <div key={card.id} className="p-4 border rounded flex items-center justify-between">
                                    <div>
                                        <Typography className="font-medium">{card.name}</Typography>
                                        <Typography className="text-sm text-gray-600">
                                            Type: {card.optionType}
                                            {card.optionType === "specific" && card.commonlink && (
                                                <span> | Common Link: {card.commonlink}</span>
                                            )}
                                        </Typography>
                                        {card.allowUser && card.allowUser.length > 0 && (
                                            <div className="text-sm text-gray-600">
                                                Nested Users ({card.allowUser.length}):
                                                <ul className="list-disc pl-5">
                                                    {card.allowUser.map((user, i) => (
                                                        <li key={i}>{user.email || user.phone || "Unknown"}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                        )}
                                        {card.specifyLink && card.specifyLink.length > 0 && (
                                            <div className="text-sm text-gray-600">
                                                Specific Users ({card.specifyLink.length}):
                                                <ul className="list-disc pl-5">
                                                    {card.specifyLink.map((user, i) => (
                                                        <li key={i}>
                                                            {user.email || user.phone || "Unknown"} | Link: {user.link || "None"}
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                                        ...
                                        {card.img && (
                                            <img
                                                src={card.imgUrl || URL.createObjectURL(card.img)}
                                                alt={card.name}
                                                className="mt-2 w-16 h-16 object-cover rounded"
                                            />
                                        )}
                                    </div>
                                    <Tooltip title="Card Added" arrow>
...
export default First_My_Dashboard_Add;