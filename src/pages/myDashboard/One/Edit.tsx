...
import { my_dashboard_1_mod, my_dashboard_1_user_card_details, my_dashboard_all_users } from "../../../api/config";
    ...
    useEffect(() => {
        ...
        const fetchData = async (retries = 3, delay = 1000): Promise<void> => {
            try {
                ...
                // Fetch all users
                const usersRes = await axios.get(`${my_dashboard_all_users}/${editCardId}`);
                const usersData = Array.isArray(usersRes.data) ? usersRes.data : usersRes.data.users || usersRes.data.data || [];
                setAllUsers(usersData);

                // Fetch card data
                const cardRes = await axios.get(`${my_dashboard_1_user_card_details}/${dashboardId_1}`);
                if (process.env.NODE_ENV === "development") {
        ...
        fetchData();
    }, [editCardId, dashboardId_1, setValue, navigate]);
    ...
    const handleExcelUpload = useCallback(
        async (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (optionType === "specific" && !commonlink) {
                toast.error("Common link is required when adding specific users.", {
                    position: "top-right",
                    autoClose: 5000,
                });
                return;
            ...
            if (!allUsers.length) {
                toast.error("No users available for matching.", {
                    position: "top-right",
                    autoClose: 5000,
                });
                return;
            ...
            for (const file of Array.from(files)) {
                ...
                if (!file.name.endsWith(".xlsx") && !file.name.endsWith(".xls")) {
                    toast.error(`Invalid file type for ${file.name}. Use .xlsx or .xls.`, {
                        position: "top-right",
                        autoClose: 5000,
                    });
                    return;
                        ...
                        reader.onload = (event) => {
                            try {
                                ...
                                const parsedData = XLSX.utils.sheet_to_json<{ Email?: string; Phone?: string }>(sheet, {
                                    header: ["Email", "Phone"], // Match exact header case from Excel
                                    raw: false,
                                    defval: "",
                                    blankrows: false,
                                });

                                if (!parsedData.length || (!parsedData[0].Email && !parsedData[0].Phone)) {
                                    toast.error(`Invalid or empty data in ${file.name}. Expected 'Email' or 'Phone' headers.`, {
                                        position: "top-right",
                                        autoClose: 5000,
                                    });
                                    resolve();
                                    ...
                                    .map((row) => {
                                        ...
                                        return optionType === "nested"
                                            ...
                                            : {
                                                ...
                                                // ✅ if optionType === "specific" use commonlink, else keep empty
                                                link: optionType === "specific" ? commonlink : "",
                                            };
                                    })
    ...
    const onSubmit = useCallback(
        async (data: FormData) => {
            ...
            try {
                ...
                await axios.patch(`${my_dashboard_1_mod}/${dashboardId_1}`, formData, {
                ...
                toast.success("Dashboard card updated successfully!", {
                    ...
                    autoClose: 3000,
                    onClose: () =>
                        navigate(`/my_dashboard_1/${editCardId}`)
                });
            } catch (error) {
                const errorMessage =
        ...
        [editCardId, dashboardId_1, specificUsers, nestedUsers, navigate, reset]
    );
    ...
    return (
        <>
            <ol className="flex text-gray-500 font-semibold dark:text-white-dark space-x-2">
                ...
                <li>/</li>
                <Link to={`/my_dashboard_1/${editCardId}`}>
            ...
            <div className="p-6 max-w-xl mx-auto bg-white rounded-2xl shadow-lg">
                <div className="flex justify-between px-2">
                    ...
                    <ClearIcon
                        onClick={() => navigate(`/my_dashboard_1/${editCardId}`)}
