...
import { my_dashboard_3, my_dashboard_all_users_2 } from "../../../api/config";
...
const Third_My_Dashboard_Add: React.FC = () => {
    const { root_Card_3 } = useParams<{ root_Card_3: string }>();
    const dashboardId = root_Card_3 || "default-dashboard-id";
    ...
    console.log("🚀 ~ Third_My_Dashboard_Add ~ location ===:", location)
    // Log dashboardId for debugging
    console.log("🚀 ~ First_My_Dashboard_Add ~ dashboardId:", dashboardId);
    const nested_1 = location.state.d1 || "";
    console.log("🚀 ~ Third_My_Dashboard_Add ~ nested_1:", nested_1)
    const nested_2 = location.state.d2 || "";
    console.log("🚀 ~ Third_My_Dashboard_Add ~ nested_2:", nested_2)
    const nested_3 = location.state.d3 || "";
    console.log("🚀 ~ Third_My_Dashboard_Add ~ nested_2:", nested_3)
    // Fetch all available users on mount
    useEffect(() => {
        const fetchAllUsers = async () => {
            try {
                const response = await axios.get(`${my_dashboard_all_users_2}/${dashboardId}`);
    ...
    const onSubmit = async (data: FormData) => {
        try {
            ...
            toast.success("Dashboard card added successfully!", {
                position: "top-right",
                autoClose: 3000,
                onClose: () => navigate(`/my_dashboard_3/${dashboardId}`),
            });
        } catch (error) {
...
export default Third_My_Dashboard_Add;