...
import { Add, Delete, Save, ArrowBack, Clear, UploadFile, Download } from "@mui/icons-material";
...
function EditMamber() {
    const { mamberId } = useParams(); // get id from route
    ...
    const fetchGroup = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${MamberAPI}/${mamberId}`);
            if (res.data.success) {
                setGroupData({
                    typeName: res.data.member.typeName,
                    users:
                        res.data.member.users.length > 0
                            ? res.data.member.users.map((u: User) => ({
                                name: u.name || "",
                                email: u.email || "",
                                phone: u.phone || ""
                            }))
            ...
            toast.error("Failed to load member group");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.patch(`${MamberAPI}/${mamberId}`, groupData);
            if (res.data.success) {
        ...
        <div className="p-6 max-w-4xl mx-auto">
            ...
            <Paper elevation={4} className="p-8 rounded-xl">
                ...
                <Button
                    startIcon={<ArrowBack />}
                    variant="text"
                    onClick={() => navigate("/my_dashboard/mamber")}
                    sx={{ mb: 3 }}
                >
                    Back
                </Button>
                ...
                <TextField
                    ...
                    onChange={(e) =>
                        setGroupData({ ...groupData, typeName: e.target.value })
                    }
                    sx={{ mb: 4 }}
                />
